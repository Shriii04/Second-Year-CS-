...
corpus <- tm_map(corpus, content_transformer(tolower)) # transform to low case
corpus <- tm_map(corpus, PlainTextDocument, lazy = T) # create a plain text document
...
t1<-table(test_data_words_df$airline_sentiment, log_pred>0.5)
accuracy1<-sum(diag(t1))/sum(t1)
precision1<-t1[2,2]/sum(t1[,2])
specificity1<-t1[1,1]/sum(t1[1,])
sensitivity1<- t1[2,2]/sum(t1[2,])


# train naive bayes model 
library(e1071)
naive_model <- naiveBayes(airline_sentiment~., data = train_data_words_df)
naive_predict <- predict(naive_model, test_data_words_df)
t<-table(naive_predict, true = test_data_words_df$airline_sentiment)
...
library("rpart")
library(rpart.plot)
tree<-rpart(airline_sentiment~.,data=train_data_words_df,method = 'class')
rpart.plot(tree)
prediction<-predict(tree,test_data_words_df,type='class')
confusionMatrix(prediction,as.factor(test_data_words_df$airline_sentiment))
summary(tree)
